 ...
 * Return: 0 if first n bytes of s1=s2, otherwise non-zero
 */
...
/**
 * _close- close a file descriptor and print an error message
...
/**
 * _read- read from a file and print an error messaage upon a failure
...
void elf_magic(const unsigned char *buffer)
{
	...
	if (_strcmp((const char *) buffer, ELFMAG, 4))
	{
 ...
 * @buffer: the ELF header
 * @big_endian: endianness (big endian if non-zer0)
 */
...
/**
 * elf_type- print entry point address
 * @buffer: string containing the entry point address
 * @bit_mode: bit mode (32 or 64)
 * @big_endian: endianness (big endian if non-zer)
 */
